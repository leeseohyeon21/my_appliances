...
class RentalProductPage extends StatelessWidget {
  const RentalProductPage({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return FutureBuilder<List<LentalProductModel>>(
            future: LentalProductService().getItems(),
  ...
  ListView _listView(double imgSize, List<LentalProductModel> products) {
    return ListView.separated(
      separatorBuilder: (context, index) => SizedBox(height: common_bg_padding),
      padding: EdgeInsets.all(common_bg_padding),
      itemCount: products.length,
      itemBuilder: (context, index) {
        LentalProductModel product = products[index];
        return InkWell(
          onTap: () {
            context.push('/$LOCATION_RENTAL_PRODUCT/:${product.productKey}');
          },
          child: Column(
